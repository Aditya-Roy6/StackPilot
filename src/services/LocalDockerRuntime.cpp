...
std::string LocalDockerRuntime::makeRunCommand(const std::string& containerName,
    ...
    return
        ...
        "fi; "
        "if [ -n \"$host_port\" ]; then "
        "code=$(curl -s -o /dev/null -w \"%{http_code}\" --max-time 3 \"http://127.0.0.1:$host_port/\" 2>/dev/null || true); "
        "case \"$code\" in "
        ...
        "  *) "
        "    code_alt=$(curl -s -o /dev/null -w \"%{http_code}\" --max-time 3 \"http://host.docker.internal:$host_port/\" 2>/dev/null || true); "
        "    case \"$code_alt\" in "
        "      2*|3*|401|403|404) ready=1; break ;; "
